...
<div class="banner">
  <section class="box">
    ...
    <div class="avatar"><a href="#"><span>陈政</span></a> </div>
  </section>
</div>
<div class="template">
  <div class="box">
    <h3>
      <p><span>站在</span>推荐 Recommend</p>
    </h3>
    <ul>
      @foreach($hot as $h)
      <li><a href="{{url('a/'.$h->art_id)}}" target="_blank"><img src="{{asset('/').$h->art_thumb}}"></a><span>{{$h->art_title}}</span></li>
      @endforeach
    </ul>
  </div>
</div>
<article>
  ...
  <aside class="right">
    ...
    <ul class="website">
      @foreach($links as $l)
      <li><a href="{{asset('/').$l->link_url}}">{{$l->link_name}}</a></li>
      @endforeach
    </ul> 
    </div>
    </aside>
